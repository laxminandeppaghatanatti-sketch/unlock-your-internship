...
def get_targets():
    ...
    for t in targets:
        ...
        eligibility = conn.execute(
            "SELECT id, requirement, met FROM target_eligibility WHERE target_id = ?", (t["id"],)
        ).fetchall()
        result.append({
            "id": t["id"],
            ...
            "eligibility": [{"id": e["id"], "requirement": e["requirement"], "met": bool(e["met"])} for e in eligibility]
        })
...
def delete_target(target_id):
    ...
    conn.execute("DELETE FROM target_eligibility WHERE target_id = ?", (target_id,))
